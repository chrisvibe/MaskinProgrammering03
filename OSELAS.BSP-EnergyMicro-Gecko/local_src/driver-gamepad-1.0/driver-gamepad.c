...
static ssize_t
my_write (struct file *filp, char __user * buff, size_t count, loff_t * offp)
{
  debugStr ("Writing");
  return 0;
}
...
static int
exer_fasync (int fd, struct file *pfile, int mode)
{
...
GPIO_interrupt (int irq, void *dev_id, struct pt_regs *regs)
{
  ...
  iowrite32 (GPIO_IF_res, (gpioMapReturn + GPIO_IFC_OFFSET));
...
gamepad_init (void)
{
  ...
  if (request_mem_region (CMU_ADDR_START, CMU_ADDR_SIZE, "CMU") == NULL)
    {
      printk (KERN_WARNING
	      "An error occured! Could not reserve memory region for CMU\n");
      return 1;
    }
  ...
  gpioMapReturn =
    ioremap_nocache ((resource_size_t) GPIO_ADDR_START, GPIO_ADDR_SIZE);
  printk ("Initializing io memory remap for CMU\n");
  cmuMapReturn =
    ioremap_nocache ((resource_size_t) CMU_ADDR_START, CMU_ADDR_SIZE);
  ...
  if (cdev_result < 0)
    {
      printk (KERN_WARNING
	      "Gamepad driver: Failed to add character device\n");
    }
  ...
  // Button setup 
  printk
    ("Writing to gpio to enable buttons. Using GPIO mode low offset %d \n",
     GPIO_MODEL_OFFSET);
  ...
  printk ("Enabling interrupt for even GPIO irq number %d\n",
	  GPIO_EVEN_IRQ_NUM);
  if (request_irq
      (GPIO_EVEN_IRQ_NUM, GPIO_interrupt, 0, "GPIO IRQ even", NULL) < 0)
    {
  ...
  if (request_irq (GPIO_ODD_IRQ_NUM,
		   GPIO_interrupt, 0, "GPIO IRQ odd", NULL) < 0)
    {
  ...
  iowrite32 ((unsigned int) 0xff, (gpioMapReturn + GPIO_EXTIRISE_OFFSET));

  //*GPIO_EXTIFALL = 0xff;
  printk ("Setting gpio extifall\n");
  iowrite32 ((unsigned int) 0xff, (gpioMapReturn + GPIO_EXTIFALL_OFFSET));
  ...
  gpio2 = gpio1 | 0xff;
  iowrite32 ((unsigned int) gpio2, (gpioMapReturn + GPIO_IEN_OFFSET));




  printk
    ("Init function complete, driver should now be visible under /dev\n");
  return 0;
}
