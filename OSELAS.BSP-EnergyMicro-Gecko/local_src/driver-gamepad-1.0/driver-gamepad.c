...
static int __init gamepad_init(void)
{
  ...
  gpioMapReturn = (uint32_t) ioremap_nocache((resource_size_t) GPIO_ADDR_START, GPIO_ADDR_SIZE);
  printk("Initializing io memory remap for CMU\n");
  cmuMapReturn = (uint32_t) ioremap_nocache((resource_size_t) CMU_ADDR_START, CMU_ADDR_SIZE);
  ...
  CMU_HFPER = ioread32((uint32_t*)(cmuMapReturn + CMU_HFPERCLKEN0_OFFSET));
  result = CMU_HFPER | CMU2_HFPERCLKEN0_GPIO;
  printk("Writing result: %d to cmu\n", result);
  iowrite32(result, (uint32_t*)(cmuMapReturn + CMU_HFPERCLKEN0_OFFSET));
  ...
  gpio1 = ioread32((uint32_t*)(gpioMapReturn + GPIO_IEN_OFFSET)); 
  gpio2 = gpio1 | 0xff;
  iowrite32(
      (unsigned int) gpio2,
